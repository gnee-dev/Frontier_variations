...
- **Hover names.** Static points cover the hero, outside the copy and the cards, with the same marker and name card as Variation 4, and the same faint pixel grid of boxes (3.5% opacity). The grid fades out behind the headline and domain field, under the nav and toward the bottom, so it only lightly textures the video.
