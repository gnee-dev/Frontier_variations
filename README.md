...
- **Layout.** This follows the second Figma file's layout with smaller type: one centred 674px stack of the 56px headline, the domain field (48px text) with Join Frontier inside it (17.5px above and below the button, 16px to its right), the 16px highlighted bonus line with the live countdown, and the 14px subtitle. Every button on this tab is the same 40px button with a 16px label. Each is one line with a 1.2 line height, and the gaps are 24px, 20px and 24px. On screens too narrow for the 674px field, the headline scales down in script to match the field's width. Three compact stat cards sit at the bottom.
