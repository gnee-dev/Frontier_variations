...
- **Hover names.** Static points cover the hero, outside the copy and the cards, with the same marker and name card as Variation 4, and a faint continuous grid of 1px lines (4.5% opacity) with one 28px cell per point (22px on phones). The hovered cell is outlined exactly on the grid lines, with a faint fill and a small centre pixel. The grid fades out behind the headline and domain field, under the nav and toward the bottom, so it only lightly textures the video.
