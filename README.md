...
assets/
  ...
  media/hero-04.mp4 / .webm     # Variation 4 / 4b video, full size: 12s silent ocean loop, 1440x1076 (H.264 + VP9 WebM)
  media/hero-04-tablet.*        # 04 frame on tablets: 1440x990 (16:11) cut
  media/hero-04-phone.*         # 04 frame on phones: 720x610 cut
  media/hero-04b-tablet.*       # 04b on tall tablet screens: 800x1076 cut around the ring
  media/hero-04b-phone.*        # 04b on phones: 612x1076 cut around the ring
  media/*-poster.jpg            # first frame of each file, shown until it plays
```
...
- **Frame.** It plays a silent 12-second loop of an ocean at dusk under a glowing ring. Each screen size loads only the file cut for it: `hero-04` (1440×1076) on laptops and desktops, `hero-04-tablet` (1440×990, 16:11) from 561 to 960px, and `hero-04-phone` (720×610) at 560px and below; H.264 MP4 where the browser can play it, VP9 WebM otherwise. The `<video>` has `preload="none"` and the script sets the right file, so nothing else downloads. Its first frame shows until it starts. It plays only while tab 04 is on, pauses on other tabs, and stays on the poster for reduced motion. The files are cut from one 1440×1076 master, so they can't go larger than that without upscaling. Corner labels, from the reference: a "Live feed" chip with a green live dot at the top left, three small uppercase lines at the bottom left (hidden on phones), and a Watch brief button at the bottom right. Soft darkening at the top and bottom edges keeps them legible.
...
- **Hover names.** As in the other variations, static points (28px apart, 22px on phones) cover the frame. Each point holds one name, shown with a small square marker until the cursor moves to another point. The labels and the button are kept clear. The points are drawn as a faint pixel grid (4% opacity) of the marker's own 17px boxes, so the marker always lands exactly on a grid box; it's a quiet texture, not a feature.
...
- **Video.** It uses the same footage, in the file cut for the hero's shape: the full `hero-04` on landscape screens (hero wider than 1.1:1), `hero-04b-tablet` (800×1076, cut around the ring) on tall tablet screens, and `hero-04b-phone` (612×1076) on phones. `fitVideo()` in `js/variation-4-video-frame.js` knows where the ring and the horizon sit in each file, and sizes and places it on every layout change so that the horizon sits just above the stat cards (about 40px; 28px on phones), the ring sits clear below the nav bar (at least 28px) and fully on screen, and the video is as large as that allows: filling the width on laptops and desktops, a little narrower with faded sides on very wide screens, and grown until the ring sits about 48px under the nav on tall screens. Its top and bottom edges fade in softly. Checked at 17 sizes from 360×640 to 2560×1080. It plays only while tab 04b is on, pauses on other tabs and stays on the poster for reduced motion.
- **Hover names.** Static points cover the hero, outside the copy and the cards, with the same marker and name card as Variation 4, and a faint continuous grid of 1px lines (4.5% opacity) with one 20px cell per point (16px on phones). The cell under the cursor is highlighted in the accent orange, exactly on the grid lines, with a small centre pixel; it shows the cell's name, which stays while the cursor rests in it. While the cursor moves, the last three cells it crossed trail behind it in orange at falling opacity (55%, 32%, 16%; skipped cells are filled in on fast moves), and the trail fades out about a tenth of a second after the cursor stops, leaving only the highlighted cell. The grid fades out behind the headline and domain field, under the nav and toward the bottom, so it only lightly textures the video.
